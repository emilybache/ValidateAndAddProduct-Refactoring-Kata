...
def validate_and_add(product_data: ProductFormData, db):
    ...
    product.product_range = ProductRange.BUDGET
    if product_data.packaging_recyclable:
        product.product_range = ProductRange.PROFESSIONAL
    ...
    if not product_data.packaging_recyclable and product.product_range == ProductRange.QUEEN:
        return Response(0, -1, "Error - failed quality check for Queen Range")
